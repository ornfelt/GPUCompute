...
// Define hardware performance mode if not already defined.
...
// Uniform declarations – update these from your application.
uniform vec3 iResolution; // (width, height, 1.0) e.g. window size
...
//--------------------------------------------------------------------
// Inigo Quilez's code for geometric orbit traps in the Mandelbrot set.
// (Copyright Inigo Quilez, 2015 - see shader comments for usage restrictions)
...
// Helper: sphere normal function.
vec3 sphNormal( in vec3 pos, in vec4 sph )
{
    return (pos - sph.xyz) / sph.w;
}

// fancyCube: sample a texture from three directions with a bias.
// This function uses the built-in texture() function with a bias value.
...
// Define a sphere used in rendering.
...
float rayMarch( in vec3 ro, in vec3 rd, float tmax )
{
    ...
    // Bounding plane: assume ground at y = 1.0.
...
vec3 render( in vec3 ro, in vec3 rd )
{
    ...
    // Raymarch for additional detail.
    float tmax = 20.0;
    if( t > 0.0 ) tmax = t; 
...
//--------------------------------------------------------------------
// mainImage: standard Shadertoy entry point.
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    {
        // Pixel coordinates with subpixel offset.
...
// Optional: VR version of mainImage.
void mainVR( out vec4 fragColor, in vec2 fragCoord, in vec3 fragRayOri, in vec3 fragRayDir )
{
    ...
    vec3 rt = vec3( 1.0, 0.0, 0.0 );
    mat3 cam = setCamera( ro, rt, 0.35 );
    fragColor = vec4( render( ro + cam * fragRayOri, cam * fragRayDir ), 1.0 );
}

//--------------------------------------------------------------------
// Standard fragment shader entry point.
out vec4 finalColor;
void main() {
